...
class Dataloader(torch.utils.data.Dataset):
    ...
    def collate_fn(self, batch):
        X, Y, NEIGHBOR, IDS, FRAMES = [], [], [], [], []
        for item in batch:
            hist, future, neighbor, agent_id, frame_id = item
        ...
        return x, y, neighbor, IDS, FRAMES
    ...
    def load(self, filename, inclusive_groups):
        ...
        while tid0 < e-horizon:
            ...
            if len(idx):
                ...
                for i in range(len(idx)):
                    ...
                    future = agents[self.ob_horizon:valid_horizon,i,:2]  # L_pred x 2
                    neighbor = agents[:valid_horizon, [d for d in range(agents.shape[1]) if d != i]] # L x (N-1) x 6
                    agent_id = idx[i]  # 加入 agent_id
                    start_frame = time[tid0 + self.ob_horizon]  # 预测起始时间
                    traj.append((hist, future, neighbor, agent_id, start_frame))
        ...
        items = []
        for hist, future, neighbor, agent_id, start_frame in traj:
            hist = np.float32(hist)
            future = np.float32(future)
            neighbor = np.float32(neighbor)
            items.append((hist, future, neighbor, agent_id, start_frame))
